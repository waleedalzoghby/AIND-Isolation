    ...
    def wrapUnitTest(testcase):
        ...
        def testWrapper(self):
            ...
            try:
                ...
                raise TimeoutError("Test aborted due to timeout. Test was " + "expected to finish in less than {} second(s).".format(time_limit))
...
class CounterBoard(isolation.Board):
    ...
    def copy(self):
        ...
        new_board.__inactive_player__ = self.inactive_player
...
class Project1Test(unittest.TestCase):
    ...
    def initAUT(self, depth, eval_fn, iterative=False,
        ...
        ##MinimaxPlayer
        agentUT = game_agent.AlphaBetaPlayer(depth, eval_fn) #MinimaxPlayer
    ...
    def test_minimax_interface(self):
        ...
        agentUT = game_agent.MinimaxPlayer(test_depth, heuristic) #, iterative_search, search_method)
        ...
        for move in board.get_legal_moves():
            ...
            self.assertTrue(type(v) == float,
                            ("Minimax function should return a floating " + "point value approximating the score for the " + "branch being searched."))

    #timeout(5)
    @unittest.skip("Skip alphabeta test.") # Uncomment this line to skip test
    def test_alphabeta_interface(self):
        """ Test MinimaxPlayer.alphabeta interface with simple input """
        h, w = 9, 9  # board size
        test_depth = 1
        ...
        # create a player agent & a game board
        agentUT = game_agent.AlphaBetaPlayer(test_depth, heuristic) #, iterative_search, search_method)
        agentUT.time_left = lambda: 99  # ignore timeout for fixed-depth search
        board = isolation.Board(agentUT, 'null_agent', w, h)
        ...
        for move in board.get_legal_moves():
            ...
            self.assertTrue(type(v) == float,
                            ("Alpha Beta function should return a floating " + "point value approximating the score for the " + "branch being searched."))
    ...
    def test_get_move_interface(self):
        ...
        self.assertIn(move, legal_moves,
                      ("The get_move() function failed as player 1 on an " + "empty board. It should return coordinates on the " + "game board for the location of the agent's next " + "move. The move must be one of the legal moves on " + "the current game board."))
        ...
        move = agentUT.get_move(board,lambda: 99)
        self.assertIn(move, legal_moves,
                      ("The get_move() function failed making the first " + "move as player 2 on a new board. It should return " + "coordinates on the game board for the location " + "of the agent's next move. The move must be one " + "of the legal moves on the current game board."))
        ...
        move = agentUT.get_move(board,lambda: 99)
        self.assertIn(move, legal_moves,
                      ("The get_move() function failed as player 1 on a " + "game in progress. It should return coordinates on" + "the game board for the location of the agent's " + "next move. The move must be one of the legal moves " + "on the current game board."))

    #@timeout(5)
    @unittest.skip("Skip minimax test.")  # Uncomment this line to skip test
    def test_minimax(self):
        """ Test MinimaxPlayer.minimax
        ...
        # (3, 1), (3, 5), (4, 2), (4, 4)].  The search function will pick one
        # of
        ...
        for idx in range(5):
            ...
            self.assertTrue(num_unique_valid, UNEXPECTED_VISIT.format(method, test_depth, counts[idx][1], board.counts[1]))

            self.assertIn(move, expected_moves[idx // 2], WRONG_MOVE.format(method, test_depth, expected_moves[idx // 2], move))

    #@timeout(20)
    #@unittest.skip("Skip alpha-beta test.") # Uncomment this line to skip test
    def test_alphabeta(self):
        """ Test AlphaBetaPlayer.alphabeta
        This test uses a scoring function that returns a constant value based
        ...
        """
        h, w = 101,101 #101, 101 # board size
        starting_location = (50,50) #(50, 50)
        adversary_location = (0, 0)  # top left corner
        iterative_search = False
        method = "alphabeta"
        ...
        for idx in range(len(counts)):
            ...
            #print(WRONG_NUM_EXPLORED)
            #print(str(test_depth))
            #print(str(num_explored_valid))
            ##print(str(num_unique_valid))
            #print(str(counts[idx][0]))
            #print(str(board.counts[0]))


            #print("==================================================")


            #print(UNEXPECTED_VISIT)
            #print(str(test_depth))
            #print(str(num_unique_valid))
            #print(str(counts[idx][1]))
            #print(str(board.counts[1]))
            ...
            WRONG_NUM_EXPLORED.format(method, test_depth, counts[idx][0],
            board.counts[0]))
            self.assertTrue(num_unique_valid, UNEXPECTED_VISIT.format(method,
            test_depth, counts[idx][1], board.counts[1]))
            self.assertIn(move, first_branch, WRONG_MOVE.format(method,
            test_depth, first_branch, move))


    #@timeout(20)
    @unittest.skip("Skip iterative deepening test.") # Uncomment this line to
    #skip test
    def test_get_move(self):
        ...
        for idx in range(len(origins)):
            ...
            agentUT, board = self.initAUT(-1, eval_fn, True, method,
                                          ...
                                          w, h)
            legal_moves = board.get_legal_moves()
            chosen_move = agentUT.get_move(board, timer.time_left) # legal_moves,
